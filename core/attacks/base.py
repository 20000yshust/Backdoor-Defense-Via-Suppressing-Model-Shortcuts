...
#
#
# def distinguishdata(correct_d):
#     for i in range(correct_d.size(0)):
#         if correct_d[i]>=1:
#             ptrue.append(i)
#         else:
#             pfalse.append(i)
#
# def my_accuracy(output, target, topk=(1,)):
...
#         if k==1:
#             correct_d=correct[:k].contiguous().float().sum(0)
#             # print(correct[:k].contiguous().float().sum(0))
#             distinguishdata(correct_d)
#     return res
#
...
#                 self.model.train()
#
#     def _test(self, dataset, device, batch_size=16, num_workers=1, model=None):
#         if model is None:
#             model = self.model
#         else:
#             model = model
...
#
#
#     def myadd_test(self, dataset, device, batch_size=16, num_workers=1, model=None):
#         global ptrue
#         global pfalse
#         if model is None:
#             model = self.model
...
#                 batch_img = batch_img.cpu()
#                 for index in ptrue:
#                     if (index<batch_size and index>=0):
#                         predict_digits_true.append(batch_img[index].reshape(1,10))
#                         labels_true.append(batch_label[index].reshape(1,1))
#                 ptrue=[x - batch_size for x in ptrue]
#                 for index in pfalse:
#                     if (index<batch_size and index>=0):
#                         predict_digits_false.append(batch_img[index].reshape(1,10))
#                         labels_false.append(batch_label[index].reshape(1,1))
#                 pfalse=[x - batch_size for x in pfalse]
#             predict_digits_true = torch.cat(predict_digits_true, dim=0)
#             # print(predict_digits_true.shape)
#             labels_true = torch.cat(labels_true, dim=0)
#             # print(labels_true.shape)
#             predict_digits_false = torch.cat(predict_digits_false, dim=0)
#             labels_false = torch.cat(labels_false, dim=0)
#             return predict_digits_true, labels_true,predict_digits_false,labels_false
#
#
...
#             total_num = labels.size(0)
#             prec1, prec5 = my_accuracy(predict_digits, labels, topk=(1, 5))
#             top1_correct = int(round(prec1.item() / 100.0 * total_num))
#             top5_correct = int(round(prec5.item() / 100.0 * total_num))
#             msg = "==========Test result on benign test dataset==========\n" + \
#                   time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + \
#                   f"Top-1 correct / Total: {top1_correct}/{total_num}, Top-1 accuracy: {top1_correct/total_num}, Top-5 correct / Total: {top5_correct}/{total_num}, Top-5 accuracy: {top5_correct/total_num} time: {time.time()-last_time}\n"
#             k=[]
#             k.append(top1_correct/total_num)
#             k.append(top5_correct/total_num)
#             log(msg)
#
...
#             last_time = time.time()
#             # test result on poisoned test dataset
#             predict_digits_true, labels_true,predict_digits_false,labels_false = self.myadd_test(poisoned_test_dataset, device, self.current_schedule['batch_size'], self.current_schedule['num_workers'], model)
#             total_num_true = labels_true.size(0)
#             print(total_num_true)
#             total_num_false = labels_false.size(0)
#             prec1_true, prec5_true = accuracy(predict_digits_true, labels_true, topk=(1, 5))
#             top1_correct_true= int(round(prec1_true.item() / 100.0 * total_num_true))
#             top5_correct_true= int(round(prec5_true.item() / 100.0 * total_num_true))
#             prec1_false, prec5_false = accuracy(predict_digits_false, labels_false, topk=(1, 5))
#             top1_correct_false= int(round(prec1_false.item() / 100.0 * total_num_false))
#             top5_correct_false= int(round(prec5_false.item() / 100.0 * total_num_false))
#             msg = "==========Test result on poisoned test dataset==========\n" + \
#                   time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + \
#                   f"Top-1 correct_true / Total: {top1_correct_true}/{total_num_true}, Top-1 accuracy: {top1_correct_true/total_num_true}, Top-5 correct / Total: {top5_correct_true}/{total_num_true}, Top-5 accuracy: {top5_correct_true/total_num_true}, time: {time.time()-last_time}\n"
#             msg2 = "==========Test result on poisoned test dataset==========\n" + \
#                   time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + \
#                   f"Top-1 correct_false / Total: {top1_correct_false}/{total_num_false}, Top-1 accuracy: {top1_correct_false/total_num_false}, Top-5 correct / Total: {top5_correct_false}/{total_num_false}, Top-5 accuracy: {top5_correct_false/total_num_false}, time: {time.time()-last_time}\n"
#             k.append(top1_correct_true/total_num_true)
#             k.append(top1_correct_false/total_num_false)
#             self.testdata.append(k)
#             log(msg)
#             log(msg2)
